...
```
* And finally, you have to add/modify specific code in the `usbd_cdc_if.c` and `usbd_cdc_if.h` files. To make things simple, you will find in the `cube-mx-custom-files` the modified files for this driver. You only have to generate the files (as often as you want) with the USB peripheral(s) you want to use and then activate the USB peripheral(s) you need by commenting out one or both of the following defines:
```c
#define USBD_DCD_FS
#define USBD_DCD_HS
```
