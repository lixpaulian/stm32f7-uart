...
This package includes "glue" code between µOS++ and the ST provided HAL and Middleware libraries. One of the goals of this project was to use code generated by the CubeMX code generator with a minimum of customizations. Currently the package includes an UART driver and an USB DCD Device driver (Virtual Com Port).
...
## Version
* UART 1.31 (21 January 2018)
* VCP 0.6 (21 January 2018)
...
* There is a much discussed issue with a `malloc()` call in an interrupt routine. There is no universal solution to this issue, unless a large part of the USB code is re-written. I choose to statically allocate two RAM areas and use one or the other depending on the USB port ID used (HS or FS). Unfortunately, the file where the change must be done has no user modifiable areas. A solution is to modify the file directly in the CubeMX repository, so that every time the code is re-generated, the user changes are not lost. When a new version comes from ST, if you have check-in your project in a repo, you can re-do the changes in the original file from ST. The changes must be done in the file `usbd_cdc.c`, located in Repository/STM32Cube_FW_F7_V1.8.0/Middlewares/ST/STM32_USB_Device_Library/Class/CDC/ (the FW version may differ). First, in the function `USBD_CDC_Init()`:
...
```
* And finally, you have to add/modify specific code in the usbd_cdc_if.c and usbd_cdc_if.h files. To make things simple, you will find in the `cube-mx-custom-files` the modified files for this driver. You only need to generate the files (as often as you want) with the USB peripheral(s) you want to use and then activate the USB peripheral(s) you need by commenting out one or both of the following defines:
```c
#define USBD_DCD_FS
#define USBD_DCD_HS
```
By now you should be already done.
...
For the VCP there is too a simple test program: this one opens the VCP and echoes back all the characters it receives. You can try it with a terminal program by typing characters that should be echoed back. More elaborate testing can be done by means of a script or a small program written in your preferred language, that opens the VCP, sends blocks of data and checks what it receives back.
